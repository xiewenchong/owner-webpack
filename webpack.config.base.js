...
var HardSourceWebpackPlugin = require('hard-source-webpack-plugin'); //为模块提供中间缓存,构建时间大约可以节约 85%
...
const baseConfig = {
    ...
    plugins: [
        ...
        // new webpack.HotModuleReplacementPlugin(), //热更新插件设置devServer的hot：true时，默认调用webpack.HotModuleReplacementPlugin插件
    ...
    resolve: {  //resolve配置项处理一些webpack的查找文件的路径
        ...
        // extensions:['*','.js','.json','.vue']    // extensions参数(定义的后缀查找文件(频率较高的文件类型优先写在前面)
        // enforceExtension参数
    },
